...
> ADR-025 for DI geometry contract refinement). Superseded ADRs live
> in the `docs/adr/superseded/` folder for historical context and are
> not binding. Reviews MUST consult the current authority
> (019/020/021/023/025) and MUST flag any implementation referencing
> the old numbers as binding. The operator shortcut SSOT moved out of
> the ADR body to `docs/operator-shortcuts.md` registry per ADR-023.
