...
**Status:** Rev-5 authored — addresses Codex Round-5 findings (1 High-risk Review Miss + 2 Quality gaps); awaiting Codex Round-6 review
...
- **A2.1 — Publication contract (Rev-2 lock-in + Rev-3 first-frame coherence).** Two slices, two timing patterns:
  ...
  - **Sync-bootstrap re-entrancy contract (Rev-4 H fix; Rev-5 H exhaustiveness lock):** the cursor-effect handler invoked from step 3a.ii is **overlay-write-only**. **Allowed:** write `overlay.preview` / `overlay.dimensionGuides` / `overlay.snap` / `overlay.cursor`; READ project store, tool state, viewport, design tokens. **Forbidden:** all "state-machine-advance" methods on the runner — currently `publishPrompt`, `advanceGenerator`, `dispatchInput`, **plus any future entrypoint added to the SSOT array below**; also forbidden: mutate `commandBar.*` slice, mutate domain state, dispatch other tools. **Enforcement (runtime — SSOT array + helper, Rev-5 H lock):** `tools/runner.ts` exposes:
    ```ts
    class ToolRunner {
      private inSyncBootstrap = false;
      // SSOT array — adding a new state-machine-advance entrypoint REQUIRES adding to this list.
      // The parameterized test (Gate REM6-P1-SyncBootstrapNoReentry) iterates this array, so
      // a new method automatically gets test coverage when added here.
      static readonly STATE_MACHINE_ADVANCE_METHODS = [
        'publishPrompt',
        'advanceGenerator',
        'dispatchInput',
      ] as const;
      private assertNotInSyncBootstrap(methodName: string): void {
        if (this.inSyncBootstrap) {
          throw new Error(
            `cursor-effect re-entered runner during sync bootstrap: ${methodName}`
          );
        }
      }
      publishPrompt(prompt: Prompt): void {
        this.assertNotInSyncBootstrap('publishPrompt');
        // ... implementation ...
      }
      advanceGenerator(): void {
        this.assertNotInSyncBootstrap('advanceGenerator');
        // ... implementation ...
      }
      dispatchInput(input: Input): void {
        this.assertNotInSyncBootstrap('dispatchInput');
        // ... implementation ...
      }
      // syncBootstrapCursorEffect sets the flag with try/finally:
      private syncBootstrapCursorEffect(tool, cursor): void {
        this.inSyncBootstrap = true;
        try { tool.cursorEffect(cursor, toolState); }
        finally { this.inSyncBootstrap = false; }
      }
    }
    ```
    The existing per-cursor-tick cursor-effect path leaves the flag at `false` — existing tools are unaffected. **Enforcement (test — Gate REM6-P1-SyncBootstrapNoReentry):** parameterized test iterates `ToolRunner.STATE_MACHINE_ADVANCE_METHODS`, asserting each method throws when called from a sync-bootstrap context. Plus exhaustiveness count gate: `rg -c "this\.assertNotInSyncBootstrap\(" tools/runner.ts` MUST be ≥ `STATE_MACHINE_ADVANCE_METHODS.length`. **Failure modes covered:** (a) forgot helper call → count gate fails AND parameterized test fails (method doesn't throw); (b) forgot to add to array → method is still runtime-guarded (helper protection intact), but its dedicated test is silently omitted — code-review responsibility to keep array current (documented in §13). **Future deviation:** §0.7 Approved Deviation Protocol — write a new ADR or extend ADR-024.
...
| `packages/editor-2d/src/tools/runner.ts` | Per Rev-2 + Rev-3 + Rev-5 publication contract: (a) on prompt-yield with `dynamicInput`, call `setDynamicInputManifest(manifest)` + reset buffers (sparse manifest publication — Phase-1 step 3a.i); (b) **synchronously trigger the per-tool cursor-effect handler with try/finally + `inSyncBootstrap` flag** to populate `overlay.dimensionGuides` BEFORE first paint (Rev-3 H2 first-frame coherence + Rev-4 H re-entrancy guard — Phase-1 step 3a.ii); (c) on tool teardown OR yielded prompt without manifest, call `clearDynamicInput()` + `setDimensionGuides(null)` (Phase-1 step 3c). **Add SSOT array `static readonly STATE_MACHINE_ADVANCE_METHODS = ['publishPrompt', 'advanceGenerator', 'dispatchInput'] as const` + private helper `assertNotInSyncBootstrap(methodName)` called at entry of each guarded method (Rev-5 H exhaustiveness lock).** Per-cursor-tick path is unchanged — guides written by per-tool cursor-effect handler (step 3b), not by the runner directly. |
...
- `packages/editor-2d/tests/click-eat-with-di.test.tsx` (NEW — Rev-3 Q fix; 3 unit tests, one per click-eat handler — handleCanvasClick / handleCanvasMouseDown / handleCanvasMouseUp — each mounts EditorRoot and asserts the handler suppresses path when DI buffers are non-empty)
...
- **Tests rework volume:** medium. ~36-43 net-new tests (substrate ~31: types ×1 + slice ×3 + runner ×5 incl. synchronous-bootstrap (Rev-3 H2) + **3 parameterized re-entrancy tests over STATE_MACHINE_ADVANCE_METHODS (Rev-5 H lock — auto-grows as array grows)** + painter ×3 + multi-pill ×4 + router ×5 + combiner helper ×7 across edge cases + click-eat handler unit tests ×3 (Rev-3 Q); per-tool generators ×4; per-tool cursor-effect ×4 (Rev-2 H1); smoke ×5 including click-eat-with-DI parity + first-frame coherence (Rev-3 H2)) + ~5 migrated. **Threshold gate (REM6-10): ≥480 (470 + ≥10 minimum).** Actual count expected to be in the ~506-516 range; informational tripwire only per Rev-1 Q2.

## 5. Architecture doc impact

| Doc | Change |
|---|---|
| `docs/adr/024-dynamic-input-manifest.md` | **NEW** — proposed. Per-prompt DI manifest contract: (a) `DynamicInputField` shape (`{ kind: 'number' \| 'distance' \| 'angle', label?: string }`) — manifest fields carry NO anchor info; (b) `DynamicInputManifest` shape (`{ fields: DynamicInputField[], combineAs: ... }`) — sparse, declarative metadata only; (c) **`DimensionGuide` discriminated union with three variants, EACH carrying flat `{x: number, y: number}` metric coords ONLY** — `linear-dim` (anchorA + anchorB + offsetCssPx), `angle-arc` (pivot + baseAngleRad + sweepAngleRad + radiusCssPx), `radius-line` (pivot + endpoint). NO reference strings (`'cursor'`, `'midpoint:p1,cursor'`), NO callbacks, NO anchor IDs (Rev-2 H1 lock-in); (d) `combineAs` policy enum (`'numberPair' \| 'point' \| 'number'`); (e) **publication contract** — `commandBar.dynamicInput.manifest` published once per prompt-yield (sparse); `overlay.dimensionGuides` written every cursor-effect tick by the per-tool cursor-effect handler (dynamic; same machinery as `overlay.cursor` / `overlay.preview`); painter `paintDimensionGuides` reads flat coords only — no resolution logic, preserves DTP-T1/T2/T6/T7 contract; (f) painter dispatch model (per-shape switch in `paintDimensionGuides`); (g) per-field buffer model + Tab-cycling focus invariant; (h) **angle-unit invariant** — typed angle field is in degrees; `combineAs: 'point'` converts to radians via `(angleDeg * Math.PI) / 180` before trig (SSOT in `packages/editor-2d/src/tools/dynamic-input-combine.ts`); (i) one-way cross-reference to ADR-023 from ADR-024's own body — ADR-023 file is NOT modified per §0.6. |
...
3. Update `tools/runner.ts` per the Rev-2 publication contract (§3 A2.1):
   - **3a. On prompt-yield with a manifest:** runner (i) calls `setDynamicInputManifest(manifest)` + resets `dynamicInput.buffers` to `Array(manifest.fields.length).fill('')` + `activeFieldIdx` to 0 (Rev-1 R2-A5 buffer-reset semantics; each prompt yield is one input session — polyline-loop iterations start with empty buffers per leg). (ii) **MUST set `inSyncBootstrap = true`, then synchronously trigger the per-tool cursor-effect handler with current `overlay.cursor`, then clear `inSyncBootstrap = false` in a `finally` block (Rev-4 H — re-entrancy guard).** Pseudocode:
     ```ts
     this.inSyncBootstrap = true;
     try {
       activeTool.cursorEffect(currentCursor, toolState);
     } finally {
       this.inSyncBootstrap = false;
     }
     ```
     This populates `overlay.dimensionGuides` BEFORE returning from the yield-side path (Rev-3 H2 — first-frame coherence) AND prevents re-entrancy (Rev-4 H + Rev-5 H exhaustiveness — overlay-write-only contract per §3 A2.1). (iii) **MUST guard ALL state-machine-advance methods via SSOT array + helper (Rev-5 H lock):** runner exposes `static readonly STATE_MACHINE_ADVANCE_METHODS = ['publishPrompt', 'advanceGenerator', 'dispatchInput'] as const`. Each named method calls `this.assertNotInSyncBootstrap('<methodName>')` at entry — helper throws `'cursor-effect re-entered runner during sync bootstrap: <methodName>'` if `inSyncBootstrap === true`. Adding a new state-machine-advance entrypoint REQUIRES adding it to the SSOT array (which auto-generates a parameterized test) AND calling the helper at entry. Both invariants enforced by Gate REM6-P1-SyncBootstrapNoReentry. (iv) **MAY add post-bootstrap runtime assertion (defense-in-depth):** `assert(overlay.dimensionGuides?.length === manifest.fields.length, 'first-frame coherence violated — cursor-effect bootstrap missing or returned wrong field count')`. Locked assumption (§3 A2.1): `overlay.cursor` is always valid at yield sites because tools yield manifests only AFTER a click that establishes cursor state.
...
Gate REM6-P1-ClickEat: each click-eat handler suppresses path when DI buffers are non-empty (Rev-3 Q — semantic unit tests; Rev-4 Q2 — locked deterministic primary assertion target)
  Architecture-significant gate (3 unit tests in NEW packages/editor-2d/tests/click-eat-with-di.test.tsx):
    ...
    Test 2: handleCanvasMouseDown — fireEvent.mouseDown(canvas).
    Test 3: handleCanvasMouseUp — fireEvent.mouseUp(canvas).
...
Gate REM6-P1-SyncBootstrapNoReentry: cursor-effect invoked from sync bootstrap is overlay-write-only — exhaustively enforced via SSOT array + helper (Rev-4 H fix; Rev-5 H exhaustiveness lock)
  Architecture-significant gate (parameterized unit test in packages/editor-2d/tests/tool-runner.test.ts):
    Pattern:
      describe('sync-bootstrap re-entrancy guard', () => {
        for (const method of ToolRunner.STATE_MACHINE_ADVANCE_METHODS) {
          it(`'sync-bootstrap cursor-effect cannot call ${method}'`, () => {
            (a) Build a fixture tool whose cursor-effect handler attempts to call runner[method](appropriateArgs);
            (b) Drive the tool to a yield point with a manifest (triggering syncBootstrapCursorEffect);
            (c) Assert: the synchronous bootstrap call throws an error matching `cursor-effect re-entered runner during sync bootstrap: ${method}`;
            (d) Assert: inSyncBootstrap flag is back to false after the throw (try/finally cleanup verified).
          });
        }
      });
    Yields 3 tests at current array size (publishPrompt + advanceGenerator + dispatchInput) — each method gets a dedicated test, none is "or"-collapsed. Adding a new state-machine-advance entrypoint to STATE_MACHINE_ADVANCE_METHODS automatically generates a corresponding test.
    Existing per-cursor-tick cursor-effect tests (Rev-2 4 cursor-effect tests) implicitly verify the flag is false during normal cursor-ticks — those must continue to pass.
  Command (architecture-significant): pnpm --filter @portplanner/editor-2d test -- tests/tool-runner
    Expected: existing tests + sync-bootstrap-coherence + 3 (or N = STATE_MACHINE_ADVANCE_METHODS.length) parameterized re-entrancy tests pass.
  Command (exhaustiveness count gate — proves each method in the SSOT array calls the helper):
    rg -c "this\.assertNotInSyncBootstrap\(" packages/editor-2d/src/tools/runner.ts
    Expected: ≥ ToolRunner.STATE_MACHINE_ADVANCE_METHODS.length (currently ≥3; grows automatically as the array grows). Failure mode: forgot to call the helper in a newly-added entrypoint → count < array.length → gate fails.
  Command (SSOT array exists + is exported): rg -n "STATE_MACHINE_ADVANCE_METHODS" packages/editor-2d/src/tools/runner.ts
    Expected: ≥1 match (declaration site; the array is `static readonly` for test-iteration access).
  Command (helper exists): rg -n "private assertNotInSyncBootstrap" packages/editor-2d/src/tools/runner.ts
    Expected: ≥1 match (single helper definition — SSOT for the throw message format).
  Command (defense-in-depth — flag lifecycle): rg -n "inSyncBootstrap" packages/editor-2d/src/tools/runner.ts
    Expected: ≥4 matches (flag declaration + set-true + set-false in finally + helper-internal check).
...
Gate REM6-10 (informational tripwire — NOT architecture-significant by itself, per Codex Round-1 Q2): Workspace test count
  Command: pnpm test
  Expected: all 6 packages pass; total ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum — actual ~36-43 across substrate + per-tool + per-tool-cursor-effect + click-eat handler tests + 3 parameterized sync-bootstrap re-entrancy tests + smoke per the §11 breakdown).
...
- [ ] **Sync-bootstrap re-entrancy contract enforced exhaustively (Rev-4 H + Rev-5 H exhaustiveness lock)** — cursor-effect handler invoked from step 3a.ii is overlay-write-only (allowed/forbidden lists locked in §3 A2.1). Runtime guard via `inSyncBootstrap` flag set/cleared with try/finally. **SSOT array `ToolRunner.STATE_MACHINE_ADVANCE_METHODS = ['publishPrompt', 'advanceGenerator', 'dispatchInput']`** + helper `assertNotInSyncBootstrap(methodName)` called at entry of each guarded method (throws `'cursor-effect re-entered runner during sync bootstrap: ${methodName}'`). Verified by Gate REM6-P1-SyncBootstrapNoReentry: parameterized test iterates the SSOT array (3 tests at current size; auto-grows when array grows) + exhaustiveness count gate `rg -c "this\.assertNotInSyncBootstrap\(" runner.ts ≥ STATE_MACHINE_ADVANCE_METHODS.length`. Existing per-cursor-tick cursor-effect path unaffected.
- [ ] **Click-eat handler unit tests pass (Rev-3 Q)** — 3 tests in `tests/click-eat-with-di.test.tsx`, one per handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`); each exercises the handler's guard in isolation without coupling to helper symbol names or comment annotations. Verified by Gate REM6-P1-ClickEat (architecture-significant gate now unit-test-driven, not grep-driven).
...
- [ ] **Workspace test count tripwire** ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum; actual ~36-43 per §11 breakdown — Rev-5 H made re-entrancy tests parameterized over the SSOT array, yielding 3 tests at current array size; auto-grows). Informational only; the architecture-significant gates are REM6-9 (named test files) + REM6-11 (cross-cutting structural) + REM6-12 (typecheck/check/build). REM6-10 demoted in Rev-1 per Codex Round-1 Q2.
...
| Click-eat gate refactor-resilience (Rev-3 Q fix; Rev-4 Q2 added deterministic primary assertion target; Rev-5 Q2 dropped fragile tertiary) | Gate REM6-P1-ClickEat is **3 unit tests** in `tests/click-eat-with-di.test.tsx` — one per handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`), each asserts the SEMANTIC outcome (handler suppresses path when DI buffers non-empty) without coupling to implementation details (helper name, regex pattern, comment annotation). **Locked deterministic two-tier gate:** Primary (MUST pass) — project-store entity count unchanged AND zundo temporal-stack length unchanged. Secondary (MUST pass) — overlay.preview unchanged. (Rev-5 Q2: tertiary "no console.error" assertion DROPPED — fragile to environment noise.) Existing smoke scenario stays for end-to-end wiring. Annotation grep simplified to count-only — pure regression-protection. |
| Sync-bootstrap re-entrancy regression — a future cursor-effect refactor could attempt to publish prompts / advance generator from inside the synchronous bootstrap path, causing recursion or state corruption (Rev-4 H fix; Rev-5 H exhaustiveness lock; Codex Round-4 + Round-5 Review Misses) | §3 A2.1 locks the contract: cursor-effect handler invoked from step 3a.ii is **overlay-write-only** with explicit allowed/forbidden lists. **Runtime enforcement (SSOT array + helper, Rev-5 H):** `tools/runner.ts` exposes `static readonly STATE_MACHINE_ADVANCE_METHODS = ['publishPrompt', 'advanceGenerator', 'dispatchInput'] as const` (extensible — adding a new entrypoint REQUIRES adding to this array) + private helper `assertNotInSyncBootstrap(methodName)` called at entry of each guarded method (throws `'cursor-effect re-entered runner during sync bootstrap: ${methodName}'`). The `inSyncBootstrap` flag is set/cleared with try/finally around the synchronous cursor-effect call. **Test enforcement (parameterized):** Gate REM6-P1-SyncBootstrapNoReentry — `for (const method of ToolRunner.STATE_MACHINE_ADVANCE_METHODS) it('sync-bootstrap cursor-effect cannot call ${method}', ...)` — 3 tests at current array size; auto-grows when array grows. Plus exhaustiveness count gate `rg -c "this\.assertNotInSyncBootstrap\(" runner.ts ≥ STATE_MACHINE_ADVANCE_METHODS.length` proves each method calls the helper. Existing per-cursor-tick cursor-effect path leaves the flag at `false` — existing tools unaffected. Future deviation requires §0.7 protocol (new ADR or extend ADR-024). |
| Future contributor adds a new state-machine-advance method to runner but forgets to add to STATE_MACHINE_ADVANCE_METHODS array (silent test-coverage gap) | Code-review responsibility — the SSOT array is declared adjacent to the method definitions in `tools/runner.ts`, making drift visually obvious during PR review. The new method itself is still runtime-guarded (helper protection intact) — primary safety preserved; only the dedicated test for that method is silently omitted. Not a behavioral regression risk; documented as accepted in §10 audit C2.10 + this risk row. **Inverse failure mode (forgot helper call but added to array):** parameterized test fires, expects throw, gets nothing → test fails loudly. Both directions caught at the level proportional to the failure severity. |
...
- **C2.6 — Test count math.** Net-new ≈ 36-43 (Rev-5 H made re-entrancy tests parameterized — 3 auto-generated from STATE_MACHINE_ADVANCE_METHODS array, replacing Rev-4's 2 manual + optional-or). Substrate breakdown: types ×1 + slice ×3 + runner ×5 (buffer-reset + synchronous-bootstrap (Rev-3 H2) + **3 parameterized re-entrancy tests (Rev-5 H — auto-grows with array)**) + painter ×3 + multi-pill ×4 + router ×5 + combiner helper ×7 (numberPair / point at `[5,30]` / point at `[5,90]` / point at `[5,0]` / point at `[5,-45]` / number / invalid → null) + click-eat handler unit tests ×3 (Rev-3 Q) ≈ 31. Per-tool generators: ×4. Per-tool cursor-effect (Rev-2 H1): ×4. Smoke: ×5 (rectangle DI / line DI / circle DI / click-eat-with-DI / first-frame coherence — Rev-3 H2). Theoretical sum ≈ 44; Vitest test-count grouping yields a practical range of ~36-43. Threshold ≥480 (470 + ≥10 minimum) passes with substantial slack regardless — and per Rev-1 Q2, REM6-10 is now an informational tripwire only.
...
- **C2.10 — Sync-bootstrap re-entrancy contract + exhaustiveness (Rev-4 H fix; Rev-5 H exhaustiveness lock; Codex Round-4 + Round-5 Review Misses).** Rev-3's synchronous-bootstrap call (step 3a.ii) introduced a callback-from-runner path. Rev-4 bounded it with an `inSyncBootstrap` flag and 2 named tests (publishPrompt + advanceGenerator), but used "and any other state-machine-advance method" without enforceable closure — Codex Round-5 correctly flagged this as exhaustiveness-under-enforced. **Rev-5 lock:** runner exposes `static readonly STATE_MACHINE_ADVANCE_METHODS = ['publishPrompt', 'advanceGenerator', 'dispatchInput'] as const` (SSOT array) + private helper `assertNotInSyncBootstrap(methodName)` called at entry of each guarded method (throws `'cursor-effect re-entered runner during sync bootstrap: ${methodName}'`). **Parameterized test** iterates the SSOT array — 3 tests at current array size, auto-grows as the array grows. **Exhaustiveness count gate:** `rg -c "this\.assertNotInSyncBootstrap\(" runner.ts` ≥ `STATE_MACHINE_ADVANCE_METHODS.length` proves each named method calls the helper. **Failure modes covered:** forgot helper call → count gate AND parameterized test fail; forgot to add to array → silent test omission only (runtime guard still active via helper — primary safety preserved; secondary test coverage is code-review responsibility). **Future deviation path (§0.7):** if a future tool legitimately needs cursor-effect to advance the state machine, write a new ADR or extend ADR-024, get user approval, ship. Not in scope for this round.
...
**Tests added by this round (~36-43 net-new — Rev-5 H made re-entrancy tests parameterized over `STATE_MACHINE_ADVANCE_METHODS` array, yielding 3 tests at current array size (auto-grows); threshold gate REM6-10 still ≥480 with substantial slack):**
...
- **Substrate runner (~5 tests; Rev-5 H made re-entrancy tests parameterized — 3 auto-generated from the SSOT array, replaces Rev-4's 2 manual + optional-or):** `tests/tool-runner.test.ts` — (1) prompt with manifest publishes to slice (with buffer-reset on each yield per Rev-1 R2-A5) + clears on teardown; (2) **`'synchronous-bootstrap-on-prompt-yield'` (Rev-3 H2)** — after a prompt-yield with a 2-field manifest and known `overlay.cursor`, assert IMMEDIATELY (before any timer/raf) that `overlay.dimensionGuides !== null` and `overlay.dimensionGuides.length === manifest.fields.length`, with the expected DimensionGuide shape; (3-5) **parameterized `'sync-bootstrap cursor-effect cannot call ${method}'` (Rev-4 H + Rev-5 H exhaustiveness lock)** — `for (const method of ToolRunner.STATE_MACHINE_ADVANCE_METHODS)` generates one test per entrypoint (currently 3: `publishPrompt` / `advanceGenerator` / `dispatchInput`). Each test mounts a fixture cursor-effect that calls `runner[method](appropriateArgs)`, drives the tool to a yield point with a manifest, asserts the synchronous bootstrap call throws an error matching `cursor-effect re-entered runner during sync bootstrap: ${method}`, and asserts `inSyncBootstrap === false` after the throw (try/finally cleanup verified). Adding a new state-machine-advance entrypoint to the SSOT array automatically generates a corresponding test — no manual test addition needed.
...
- **Substrate click-eat unit tests (~3 tests; NEW in Rev-3, assertion target locked in Rev-4 Q2; tertiary dropped in Rev-5 Q2):** `tests/click-eat-with-di.test.tsx` — one test per click-eat handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`). Each mounts EditorRoot, sets `commandBar.inputBuffer = ''` AND `commandBar.dynamicInput.buffers = ['5', '']`, snapshots `entityCountBefore` + `temporalStackLengthBefore` + `previewBefore`, fires the specific event on the canvas. **Locked deterministic two-tier gate:** Primary (MUST pass) — `projectStore.entities.size` unchanged AND `projectStore.temporal.pastStates.length` unchanged. Secondary (MUST pass) — `overlay.preview` unchanged. (Rev-5 Q2: tertiary "no `console.error`" DROPPED — fragile to environment noise.) Assertion targets abstract over project-store storage internals via exposed selectors; refactor-resilient. Replaces the Rev-2 helper-name-coupled grep gate.
...
### Paste to Codex for plan review (Round 6)
> Review this plan using `docs/procedures/Codex/02-plan-review.md`
> (Procedure 02). Apply strict evidence mode. Start from Round 6 —
> this is Rev-5 of the plan; prior rounds returned No-Go (Round-1:
> 1 Blocker / 2 High-risk / 2 Quality; Round-2: 1 High-risk Review
> Miss / 2 Quality; Round-3: 1 High-risk regression / 1 High-risk
> Review Miss / 1 Quality; Round-4: 1 High-risk Review Miss / 2
> Quality; Round-5: 1 High-risk Review Miss / 2 Quality); see the
> Revision history table at top of plan for the complete Rev-1
> through Rev-5 disposition.
>
