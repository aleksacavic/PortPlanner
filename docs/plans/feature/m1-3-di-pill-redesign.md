...
**Status:** Rev-2 authored — addresses Codex Round-2 findings (1 High-risk + 2 Quality gaps); awaiting Codex Round-3 review
...
- **A2.1 — Publication contract (Rev-2 lock-in).** Two slices, two timing patterns:
  ...
  - **Pill component reads BOTH slices** — `commandBar.dynamicInput` for buffers + activeFieldIdx + manifest field labels; `overlay.dimensionGuides[N]` for pill N's anchor coords. Multi-source reads are allowed in chrome (the painter contract restriction does not apply to React components).
...
- **Tests rework volume:** medium. ~29-36 net-new tests (substrate ~24: types ×1 + slice ×3 + runner ×1 + painter ×3 + multi-pill ×4 + router ×5 + combiner helper ×7 across edge cases; per-tool generators ×4; **per-tool cursor-effect ×4 — Rev-2 H1 lock-in**; smoke ×4 including click-eat-with-DI parity) + ~5 migrated. **Threshold gate (REM6-10): ≥480 (470 + ≥10 minimum).** Actual count expected to be in the ~499-509 range; informational tripwire only per Rev-1 Q2.
...
3. Update `tools/runner.ts` per the Rev-2 publication contract (§3 A2.1):
   - **3a. On prompt-yield with a manifest:** runner calls `setDynamicInputManifest(manifest)` + resets `dynamicInput.buffers` to `Array(manifest.fields.length).fill('')` + `activeFieldIdx` to 0 (Rev-1 R2-A5 buffer-reset semantics; each prompt yield is one input session — polyline-loop iterations start with empty buffers per leg). **Initial `overlay.dimensionGuides` set to `null`** — the per-tool cursor-effect handler will populate on the next cursor-tick.
...
Gate REM6-P1-ClickEat: each click-eat guard site references DI state (Rev-2 Q2 — tightened from Rev-1's loose count grep)
  Command (grep — per-site annotation-anchored): rg -B 1 -A 5 "M1\.3d-Rem(ediation)?-4 G2" packages/editor-2d/src/EditorRoot.tsx | rg -c "dynamicInput|hasNonEmptyDIBuffer"
  Expected: ≥3 matches (one per of the 3 click-eat annotation blocks: handleCanvasClick / handleCanvasMouseDown / handleCanvasMouseUp). The M1.3d-Rem-4 G2 comment annotation pattern was established at the 3 sites in M1.3d-Rem-4; Rev-2 keeps the annotations and verifies each adjacent guard references DI state — proving each individual handler was changed, not just that DI is mentioned somewhere in the file.
...
Gate REM6-10 (informational tripwire — NOT architecture-significant by itself, per Codex Round-1 Q2): Workspace test count
  Command: pnpm test
  Expected: all 6 packages pass; total ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum — actual ~29-36 across substrate + per-tool + per-tool-cursor-effect + smoke per the §11 breakdown).
...
- [ ] **Workspace test count tripwire** ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum; actual ~29-36 per §11 breakdown — Rev-2 added 4 cursor-effect tests). Informational only; the architecture-significant gates are REM6-9 (named test files) + REM6-11 (cross-cutting structural) + REM6-12 (typecheck/check/build). REM6-10 demoted in Rev-1 per Codex Round-1 Q2.
...
- **C2.6 — Test count math.** Net-new ≈ 29-36 (Rev-2 added 4 per-tool cursor-effect tests on top of Rev-1's ~25-32). Substrate breakdown: types ×1 + slice ×3 + runner ×1 (incl. buffer-reset semantics test per Rev-1 R2-A5) + painter ×3 + multi-pill ×4 + router ×5 + **combiner helper ×7** (numberPair / point at `[5,30]` / point at `[5,90]` / point at `[5,0]` / point at `[5,-45]` / number / invalid → null) ≈ 24. Per-tool generators: ×4. **Per-tool cursor-effect (Rev-2 H1): ×4** (rectangle / line / polyline / circle — each asserts the tool's cursor-effect handler writes the expected `DimensionGuide[]` to `overlay.dimensionGuides` given known cursor + tool state). Smoke: **×4** (rectangle DI / line DI / circle DI / `'click is eaten while DI buffer non-empty (multi-field DI parity)'`). Theoretical sum ≈ 36; Vitest test-count grouping (some it() blocks may be table-driven and counted per-row vs per-block) yields a practical range of ~29-36. Threshold ≥480 (470 + ≥10 minimum) passes with substantial slack regardless — and per Rev-1 Q2, REM6-10 is now an informational tripwire only.
...
**Tests added by this round (~29-36 net-new — Rev-2 added 4 per-tool cursor-effect tests on top of Rev-1's ~25-32; threshold gate REM6-10 still ≥480 with substantial slack):**
...
**Status:** Rev-2 authored — addresses Codex Round-2 findings (1 High-risk Review Miss + 2 Quality gaps); awaiting Codex Round-3 review

### Paste to Codex for plan review (Round 3)
> Review this plan using `docs/procedures/Codex/02-plan-review.md`
> (Procedure 02). Apply strict evidence mode. Start from Round 3 —
> this is Rev-2 of the plan; Round-1 returned No-Go (1 Blocker / 2
> High-risk / 2 Quality gaps), Round-2 returned No-Go (1 High-risk
> Review Miss / 2 Quality gaps); see the Revision history table at
> top of plan for the complete Rev-1 + Rev-2 disposition.
>
