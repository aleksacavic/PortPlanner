...
**Status:** Rev-3 authored — addresses Codex Round-3 findings (1 High-risk regression + 1 High-risk Review Miss + 1 Quality gap); awaiting Codex Round-4 review
...
3. Update `tools/runner.ts` per the Rev-2 publication contract (§3 A2.1):
   - **3a. On prompt-yield with a manifest:** runner (i) calls `setDynamicInputManifest(manifest)` + resets `dynamicInput.buffers` to `Array(manifest.fields.length).fill('')` + `activeFieldIdx` to 0 (Rev-1 R2-A5 buffer-reset semantics; each prompt yield is one input session — polyline-loop iterations start with empty buffers per leg). (ii) **Synchronously triggers the per-tool cursor-effect handler with current `overlay.cursor` (Rev-3 H2 fix — first-frame coherence).** This populates `overlay.dimensionGuides` BEFORE returning from the yield-side path, ensuring the first paint after yield sees a coherent (manifest, guides) pair. (iii) **MAY add runtime assertion (defense-in-depth):** `assert(overlay.dimensionGuides?.length === manifest.fields.length, 'first-frame coherence violated — cursor-effect bootstrap missing or returned wrong field count')`. Locked assumption (§3 A2.1): `overlay.cursor` is always valid at yield sites because tools yield manifests only AFTER a click that establishes cursor state.
...
Gate REM6-P1-ClickEat: each click-eat handler suppresses path when DI buffers are non-empty (Rev-3 Q — converted from name-coupled grep to unit-test-driven gate)
  Architecture-significant gate (3 unit tests in NEW packages/editor-2d/tests/click-eat-with-di.test.tsx):
    Test 1: handleCanvasClick — mount EditorRoot, set commandBar.inputBuffer = '', set commandBar.dynamicInput = { manifest: ..., buffers: ['5', ''], activeFieldIdx: 0 }, fireEvent.click(canvas), assert no geometry committed (no entity added to project store, no preview commit).
    Test 2: handleCanvasMouseDown — same setup, fireEvent.mouseDown(canvas), assert no geometry committed.
    Test 3: handleCanvasMouseUp — same setup, fireEvent.mouseUp(canvas), assert no geometry committed.
    Each test exercises the specific handler's guard in isolation — does not depend on helper symbol names, regex patterns, or comment annotations. Refactor-resilient.
...
Gate REM6-10 (informational tripwire — NOT architecture-significant by itself, per Codex Round-1 Q2): Workspace test count
  Command: pnpm test
  Expected: all 6 packages pass; total ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum — actual ~33-40 across substrate + per-tool + per-tool-cursor-effect + click-eat handler tests + smoke per the §11 breakdown).
...
- [ ] **Click-eat guard extended for DI buffers** — 3 sites in `EditorRoot.tsx` (handleCanvasClick / Down / Up) reference DI state in the guard expression; smoke scenario `'click is eaten while DI buffer non-empty (multi-field DI parity)'` passes. Verified by REM6-P1-ClickEat (Rev-2 tightened to per-site annotation-anchored grep).
...
- [ ] **Workspace test count tripwire** ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum; actual ~33-40 per §11 breakdown — Rev-3 added 3 click-eat handler unit tests + 1 synchronous-bootstrap unit test + 1 first-frame coherence smoke). Informational only; the architecture-significant gates are REM6-9 (named test files) + REM6-11 (cross-cutting structural) + REM6-12 (typecheck/check/build). REM6-10 demoted in Rev-1 per Codex Round-1 Q2.
...
- **C2.6 — Test count math.** Net-new ≈ 33-40 (Rev-3 added 3 click-eat handler unit tests + 1 synchronous-bootstrap unit test + 1 first-frame coherence smoke on top of Rev-2's ~29-36). Substrate breakdown: types ×1 + slice ×3 + runner ×2 (buffer-reset test + **synchronous-bootstrap test — Rev-3 H2**) + painter ×3 + multi-pill ×4 + router ×5 + **combiner helper ×7** (numberPair / point at `[5,30]` / point at `[5,90]` / point at `[5,0]` / point at `[5,-45]` / number / invalid → null) + **click-eat handler unit tests ×3 (Rev-3 Q)** ≈ 28. Per-tool generators: ×4. Per-tool cursor-effect (Rev-2 H1): ×4. Smoke: **×5** (rectangle DI / line DI / circle DI / click-eat-with-DI / **first-frame coherence — Rev-3 H2**). Theoretical sum ≈ 41; Vitest test-count grouping yields a practical range of ~33-40. Threshold ≥480 (470 + ≥10 minimum) passes with substantial slack regardless — and per Rev-1 Q2, REM6-10 is now an informational tripwire only.
...
**Tests added by this round (~33-40 net-new — Rev-3 added 3 click-eat handler unit tests + 1 synchronous-bootstrap unit test + 1 first-frame coherence smoke on top of Rev-2's ~29-36; threshold gate REM6-10 still ≥480 with substantial slack):**
...
- **Substrate runner (~2 tests; Rev-3 added synchronous-bootstrap test):** `tests/tool-runner.test.ts` — (1) prompt with manifest publishes to slice (with buffer-reset on each yield per Rev-1 R2-A5) + clears on teardown; (2) **`'synchronous-bootstrap-on-prompt-yield'` (Rev-3 H2)** — after a prompt-yield with a 2-field manifest and known `overlay.cursor`, assert IMMEDIATELY (before any timer/raf) that `overlay.dimensionGuides !== null` and `overlay.dimensionGuides.length === manifest.fields.length`, with the expected DimensionGuide shape for the fixture tool given that cursor.
...
- **Substrate click-eat unit tests (~3 tests; NEW in Rev-3 — Codex Round-3 Q fix):** `tests/click-eat-with-di.test.tsx` — one test per click-eat handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`). Each mounts EditorRoot, sets `commandBar.inputBuffer = ''` AND `commandBar.dynamicInput.buffers = ['5', '']`, fires the specific event on the canvas, asserts no geometry committed (no entity added to project store, no preview commit). Replaces the Rev-2 helper-name-coupled grep gate with semantic outcome assertions — refactor-resilient.
...
### Paste to Codex for plan review (Round 4)
> Review this plan using `docs/procedures/Codex/02-plan-review.md`
> (Procedure 02). Apply strict evidence mode. Start from Round 4 —
> this is Rev-3 of the plan; Round-1 returned No-Go (1 Blocker / 2
> High-risk / 2 Quality gaps), Round-2 returned No-Go (1 High-risk
> Review Miss / 2 Quality gaps), Round-3 returned No-Go (1 High-risk
> regression / 1 High-risk Review Miss / 1 Quality); see the Revision
> history table at top of plan for the complete Rev-1 + Rev-2 + Rev-3
> disposition.
>
