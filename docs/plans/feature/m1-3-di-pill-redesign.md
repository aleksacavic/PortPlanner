...
**Status:** Rev-4 authored — addresses Codex Round-4 findings (1 High-risk Review Miss + 2 Quality gaps); awaiting Codex Round-5 review
...
- **A2.1 — Publication contract (Rev-2 lock-in + Rev-3 first-frame coherence).** Two slices, two timing patterns:
  ...
  - **Sync-bootstrap re-entrancy contract (Rev-4 H fix):** the cursor-effect handler invoked from step 3a.ii is **overlay-write-only**. **Allowed:** write `overlay.preview` / `overlay.dimensionGuides` / `overlay.snap` / `overlay.cursor`; READ project store, tool state, viewport, design tokens. **Forbidden:** publish prompts (`runner.publishPrompt`), advance generator (`runner.advanceGenerator`), dispatch tool submit/cancel/input (`runner.dispatchInput` and equivalent), mutate `commandBar.*` slice, mutate domain state, dispatch other tools. **Enforcement (runtime):** `tools/runner.ts` maintains an internal `inSyncBootstrap: boolean` flag, set to `true` immediately before the synchronous cursor-effect call and cleared in a `finally` block (so the flag always clears even if cursor-effect throws). Each forbidden method checks the flag at entry: `if (this.inSyncBootstrap) throw new Error('cursor-effect re-entered runner during sync bootstrap')`. The existing per-cursor-tick cursor-effect path leaves the flag at `false` — existing tools are unaffected. **Enforcement (test):** Gate REM6-P1-SyncBootstrapNoReentry — 2 unit tests in `tests/tool-runner.test.ts` mount a fixture cursor-effect that tries each forbidden operation and assert the runner throws the expected error. **Future deviation:** if a future tool legitimately needs cursor-effect to advance the state machine (e.g., a chained-prompt scenario), that's a §0.7 Approved Deviation Protocol matter — write a new ADR or extend ADR-024, get user approval, ship. Not in scope for this round.
...
3. Update `tools/runner.ts` per the Rev-2 publication contract (§3 A2.1):
   - **3a. On prompt-yield with a manifest:** runner (i) calls `setDynamicInputManifest(manifest)` + resets `dynamicInput.buffers` to `Array(manifest.fields.length).fill('')` + `activeFieldIdx` to 0 (Rev-1 R2-A5 buffer-reset semantics; each prompt yield is one input session — polyline-loop iterations start with empty buffers per leg). (ii) **MUST set `inSyncBootstrap = true`, then synchronously trigger the per-tool cursor-effect handler with current `overlay.cursor`, then clear `inSyncBootstrap = false` in a `finally` block (Rev-4 H — re-entrancy guard).** Pseudocode:
     ...
     ```
     This populates `overlay.dimensionGuides` BEFORE returning from the yield-side path (Rev-3 H2 — first-frame coherence) AND prevents re-entrancy (Rev-4 H — overlay-write-only contract per §3 A2.1). (iii) **MUST guard forbidden methods (Rev-4 H):** `publishPrompt`, `advanceGenerator`, `dispatchInput` (and any other state-machine-advance method) check `this.inSyncBootstrap` at entry and throw `'cursor-effect re-entered runner during sync bootstrap'` if set. (iv) **MAY add post-bootstrap runtime assertion (defense-in-depth):** `assert(overlay.dimensionGuides?.length === manifest.fields.length, 'first-frame coherence violated — cursor-effect bootstrap missing or returned wrong field count')`. Locked assumption (§3 A2.1): `overlay.cursor` is always valid at yield sites because tools yield manifests only AFTER a click that establishes cursor state.
...
Gate REM6-P1-ClickEat: each click-eat handler suppresses path when DI buffers are non-empty (Rev-3 Q — semantic unit tests; Rev-4 Q2 — locked deterministic primary assertion target)
  Architecture-significant gate (3 unit tests in NEW packages/editor-2d/tests/click-eat-with-di.test.tsx):
    ...
    **Locked assertion order (Rev-4 Q2 — deterministic primary signal):**
      ...
      Tertiary (SHOULD pass — proves "no error path triggered"):
        d) No `console.error` invocations (spied via vi.spyOn).
...
Gate REM6-P1-SyncBootstrapNoReentry: cursor-effect invoked from sync bootstrap is overlay-write-only (Rev-4 H fix)
  Architecture-significant gate (2 unit tests in packages/editor-2d/tests/tool-runner.test.ts):
    Test 1: 'sync-bootstrap cursor-effect cannot republish prompt' —
      (a) Build a fixture tool whose cursor-effect handler attempts to call runner.publishPrompt(somePrompt);
      (b) Drive the tool to a yield point with a manifest;
      (c) Assert: the synchronous bootstrap call throws 'cursor-effect re-entered runner during sync bootstrap';
      (d) Assert: inSyncBootstrap flag is back to false after the throw (try/finally cleanup verified).
    Test 2: 'sync-bootstrap cursor-effect cannot advance generator' —
      Same pattern; cursor-effect attempts runner.advanceGenerator() (or runner.dispatchInput); assert throw.
    Both tests prove the runtime guard (step 3a.iii) catches forbidden re-entrancy regardless of which method is targeted. Existing per-cursor-tick cursor-effect tests (Rev-2 4 cursor-effect tests) implicitly verify that the flag is false during normal cursor-ticks — those tests must continue to pass.
  Command (architecture-significant): pnpm --filter @portplanner/editor-2d test -- tests/tool-runner
    Expected: existing tests + new sync-bootstrap-coherence + 2 new sync-bootstrap-no-reentry tests pass.
  Command (defense-in-depth grep): rg -n "inSyncBootstrap" packages/editor-2d/src/tools/runner.ts
    Expected: ≥4 matches (flag declaration + set-true + set-false in finally + guard checks at forbidden methods).
...
Gate REM6-10 (informational tripwire — NOT architecture-significant by itself, per Codex Round-1 Q2): Workspace test count
  Command: pnpm test
  Expected: all 6 packages pass; total ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum — actual ~35-42 across substrate + per-tool + per-tool-cursor-effect + click-eat handler tests + sync-bootstrap re-entrancy tests + smoke per the §11 breakdown).
...
- [ ] **Sync-bootstrap re-entrancy contract enforced (Rev-4 H)** — cursor-effect handler invoked from step 3a.ii is overlay-write-only (allowed/forbidden lists locked in §3 A2.1). Runtime guard via `inSyncBootstrap` flag set/cleared with try/finally; forbidden runner methods (`publishPrompt` / `advanceGenerator` / `dispatchInput`) check the flag and throw `'cursor-effect re-entered runner during sync bootstrap'`. Verified by Gate REM6-P1-SyncBootstrapNoReentry (2 unit tests in `tests/tool-runner.test.ts`: republish-prompt + advance-generator). Existing per-cursor-tick cursor-effect path unaffected.
...
- [ ] **Workspace test count tripwire** ≥ 480 (post-Round-5 baseline 470 + ≥10 net-new minimum; actual ~35-42 per §11 breakdown — Rev-4 added 2 sync-bootstrap re-entrancy unit tests on top of Rev-3's count). Informational only; the architecture-significant gates are REM6-9 (named test files) + REM6-11 (cross-cutting structural) + REM6-12 (typecheck/check/build). REM6-10 demoted in Rev-1 per Codex Round-1 Q2.
...
| Click-eat gate refactor-resilience (Rev-3 Q fix; Rev-4 Q2 added deterministic primary assertion target) | Gate REM6-P1-ClickEat is **3 unit tests** in `tests/click-eat-with-di.test.tsx` — one per handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`), each asserts the SEMANTIC outcome (handler suppresses path when DI buffers non-empty) without coupling to implementation details (helper name, regex pattern, comment annotation). **Locked deterministic primary assertion target (Rev-4 Q2):** project-store entity count unchanged AND zundo temporal-stack length unchanged. Secondary: overlay.preview unchanged. Tertiary: no `console.error`. Existing smoke scenario stays for end-to-end wiring. Annotation grep simplified to count-only — pure regression-protection. |
| Sync-bootstrap re-entrancy regression — a future cursor-effect refactor could attempt to publish prompts / advance generator from inside the synchronous bootstrap path, causing recursion or state corruption (Rev-4 H fix; Codex Round-4 Review Miss caused by Rev-3 H2's missing contract bound) | §3 A2.1 locks the contract: cursor-effect handler invoked from step 3a.ii is **overlay-write-only** with explicit allowed/forbidden lists. **Runtime enforcement:** `tools/runner.ts` maintains an `inSyncBootstrap` flag set/cleared with try/finally; forbidden methods (`publishPrompt` / `advanceGenerator` / `dispatchInput`) check the flag and throw `'cursor-effect re-entered runner during sync bootstrap'`. **Test enforcement:** Gate REM6-P1-SyncBootstrapNoReentry — 2 unit tests in `tests/tool-runner.test.ts` mount fixture cursor-effects that try forbidden operations and assert the guard throws. Existing per-cursor-tick cursor-effect path leaves the flag at `false` — existing tools unaffected. Future deviation requires §0.7 protocol (new ADR or extend ADR-024). |
...
- **C2.6 — Test count math.** Net-new ≈ 35-42 (Rev-4 added 2 sync-bootstrap re-entrancy unit tests on top of Rev-3's ~33-40). Substrate breakdown: types ×1 + slice ×3 + runner ×4 (buffer-reset + synchronous-bootstrap (Rev-3 H2) + **2 re-entrancy tests (Rev-4 H)**) + painter ×3 + multi-pill ×4 + router ×5 + combiner helper ×7 (numberPair / point at `[5,30]` / point at `[5,90]` / point at `[5,0]` / point at `[5,-45]` / number / invalid → null) + click-eat handler unit tests ×3 (Rev-3 Q) ≈ 30. Per-tool generators: ×4. Per-tool cursor-effect (Rev-2 H1): ×4. Smoke: ×5 (rectangle DI / line DI / circle DI / click-eat-with-DI / first-frame coherence — Rev-3 H2). Theoretical sum ≈ 43; Vitest test-count grouping yields a practical range of ~35-42. Threshold ≥480 (470 + ≥10 minimum) passes with substantial slack regardless — and per Rev-1 Q2, REM6-10 is now an informational tripwire only.
...
- **C2.10 — Sync-bootstrap re-entrancy contract (Rev-4 H fix; Codex Round-4 Review Miss).** Rev-3's synchronous-bootstrap call (step 3a.ii) introduced a callback-from-runner path that wasn't bounded — if a future cursor-effect handler called back into runner methods (publishPrompt, advanceGenerator, dispatchInput), recursion or state corruption could result. Codex Round-4 correctly flagged the missing contract. **Fix:** §3 A2.1 locks the contract to "overlay-write-only" with explicit allowed/forbidden lists. Step 3a.ii adds runtime guard: `inSyncBootstrap` flag set before sync cursor-effect call, cleared in `finally`, checked at entry of each forbidden runner method. NEW Gate REM6-P1-SyncBootstrapNoReentry — 2 unit tests in `tests/tool-runner.test.ts` mount fixture cursor-effects that try forbidden operations and assert the guard throws. Existing per-cursor-tick cursor-effect path leaves the flag at `false` — existing tools unaffected. **Future deviation path (§0.7):** if a future tool legitimately needs cursor-effect to advance the state machine, write a new ADR or extend ADR-024, get user approval, ship. Not in scope for this round.
...
**Tests added by this round (~35-42 net-new — Rev-4 added 2 sync-bootstrap re-entrancy unit tests on top of Rev-3's ~33-40; threshold gate REM6-10 still ≥480 with substantial slack):**
...
- **Substrate runner (~4 tests; Rev-4 added 2 re-entrancy tests on top of Rev-3's synchronous-bootstrap test):** `tests/tool-runner.test.ts` — (1) prompt with manifest publishes to slice (with buffer-reset on each yield per Rev-1 R2-A5) + clears on teardown; (2) **`'synchronous-bootstrap-on-prompt-yield'` (Rev-3 H2)** — after a prompt-yield with a 2-field manifest and known `overlay.cursor`, assert IMMEDIATELY (before any timer/raf) that `overlay.dimensionGuides !== null` and `overlay.dimensionGuides.length === manifest.fields.length`, with the expected DimensionGuide shape for the fixture tool given that cursor; (3) **`'sync-bootstrap cursor-effect cannot republish prompt'` (Rev-4 H)** — fixture cursor-effect tries `runner.publishPrompt(somePrompt)`; assert throw with `'cursor-effect re-entered runner during sync bootstrap'`; assert `inSyncBootstrap === false` after throw (try/finally cleanup verified); (4) **`'sync-bootstrap cursor-effect cannot advance generator'` (Rev-4 H)** — same pattern targeting `runner.advanceGenerator` (and/or `runner.dispatchInput`); assert throw with the same error.
...
- **Substrate click-eat unit tests (~3 tests; NEW in Rev-3, assertion target locked in Rev-4 Q2):** `tests/click-eat-with-di.test.tsx` — one test per click-eat handler (`handleCanvasClick` / `handleCanvasMouseDown` / `handleCanvasMouseUp`). Each mounts EditorRoot, sets `commandBar.inputBuffer = ''` AND `commandBar.dynamicInput.buffers = ['5', '']`, snapshots `entityCountBefore` + `temporalStackLengthBefore` + `previewBefore`, fires the specific event on the canvas. **Locked deterministic primary assertion target (Rev-4 Q2):** Primary (MUST pass) — `projectStore.entities.size` unchanged AND `projectStore.temporal.pastStates.length` unchanged. Secondary (MUST pass) — `overlay.preview` unchanged. Tertiary (SHOULD pass) — no `console.error`. Assertion targets abstract over project-store storage internals via exposed selectors; refactor-resilient. Replaces the Rev-2 helper-name-coupled grep gate.
...
### Paste to Codex for plan review (Round 5)
> Review this plan using `docs/procedures/Codex/02-plan-review.md`
> (Procedure 02). Apply strict evidence mode. Start from Round 5 —
> this is Rev-4 of the plan; prior rounds returned No-Go (Round-1: 1
> Blocker / 2 High-risk / 2 Quality; Round-2: 1 High-risk Review Miss
> / 2 Quality; Round-3: 1 High-risk regression / 1 High-risk Review
> Miss / 1 Quality; Round-4: 1 High-risk Review Miss / 2 Quality);
> see the Revision history table at top of plan for the complete
> Rev-1 + Rev-2 + Rev-3 + Rev-4 disposition.
>
...
> unchanged (primary), overlay.preview unchanged (secondary), no
> console.error (tertiary). Refactor-resilient via project-store's
> exposed selectors.
>
...
>   pure? (Rev-4 picks runtime guard for GR-2 simplicity; if Round-5
>   wants types, that's a Round-5 fix.)
>
> - Are there other runner methods beyond publishPrompt /
>   advanceGenerator / dispatchInput that should also check the
>   inSyncBootstrap flag? E.g., does the runner expose any other
>   state-machine-advance method?
>
