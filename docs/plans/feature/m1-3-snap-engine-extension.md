...
| 4 | 2026-04-30 | Codex Procedure 02 Round-2 review (No-Go, 9.1/10) raised one remaining blocker: schema-strategy contradiction in active text — §10 Risks row still said "Default-fill in deserializer ensures legacy points get `'circle-dot'` automatically" which conflicts with the A9 clean-break policy. Three corrections applied: (a) §4 In-scope row reframed as clean-break with the `default('circle-dot')` clarified as in-flight 1.2.0 tolerance; (b) §7 net-new tests bullet expanded from 1 → 3 cases (1.1.0 reject, 1.2.0 omit-default, 1.2.0 explicit round-trip), eliminating the "legacy point hydrates" wording; (c) §10 Risks row rewritten with explicit GR-1 + architecture-contract §0.6 framing + explicit pointer to REM8-P3-LegacyReject + REM8-P3-PointShapeDefault locks. No code or gate changes — text-only contradiction fix per Codex Round-2 §5. |
...
- Schema version bump 1.1.0 → 1.2.0 in `Project` (clean-break per A9 / I-PT-1; existing 1.1.0 saves return `LoadFailure`). New `displayShape` field on `PointPrimitiveSchema` is `z.enum([...]).default('circle-dot')` for tolerance to in-flight 1.2.0 saves omitting the field — NOT a backward-compat scaffold for 1.1.0.
- Tests:
...
- `domain` deserialize tests — 3 net-new (per Phase 3 step 6): (a) 1.1.0 project rejects with `LoadFailure` (clean-break enforced), (b) 1.2.0 project omitting `displayShape` parses with the field defaulted to `'circle-dot'` (in-flight 1.2.0 tolerance), (c) 1.2.0 project with explicit `displayShape: 'x'` round-trips bit-identically.
...
| Schema bump 1.1.0 → 1.2.0 rejects existing project files (clean-break per A9 / I-PT-1) | Per architecture-contract §0.6 preproduction posture this is the deliberate path — no migration shim, no dual-accept union. Users re-create saved projects. The `displayShape.default('circle-dot')` on `PointPrimitiveSchema` is for tolerance to **in-flight 1.2.0 saves** that omit the field (e.g., from a partial implementation snapshot during Phase 3 development), NOT a backward-compat scaffold for 1.1.0 saves. Locked by REM8-P3-LegacyReject (asserts 1.1.0 returns `LoadFailure`) + REM8-P3-PointShapeDefault (asserts 1.2.0-omit fills `'circle-dot'`). |
