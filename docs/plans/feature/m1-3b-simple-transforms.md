...
**Format note (Rev-3 per Codex Round-1 B1 + Self-Review-Miss #3):** every "Implementation site" cell uses `path:line` for existing files OR `path (NEW Phase N step M)` for files this plan creates. No phase-narrative-only cells. Every "Test that observes it" cell names an explicit test case title + boundary type (tool-generator / painter / router / domain-unit).

| User action | Expected visible result | Implementation site | Test that observes it (title + boundary) |
|---|---|---|---|
| Press `R` + Enter (no selection) | Tool activates; prompt: "Select objects" | `packages/editor-2d/src/keyboard/shortcuts.ts:36-46` (R entry NEW Phase 6) + `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, "Read selection" branch)` | tool-generator: `rotate.test.ts` "yields select prompt when selection empty" |
| Press `R` + Enter (with selection) | Tool activates; prompt: "Specify base point" | `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, selection-non-empty branch)` | tool-generator: `rotate.test.ts` "yields base-point prompt directly when selection non-empty" |
| Click base point in Rotate | Prompt: `"Specify rotation angle or [Reference]"`; live `'rotated-entities'` ghost rotating from 0° as cursor angle changes | `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, third yield with subOptions+previewBuilder)` + `packages/editor-2d/src/canvas/painters/paintPreview.ts:80-92 (NEW case 'rotated-entities' Phase 1 step 8)` + `packages/editor-2d/src/tools/types.ts:117-138 (NEW arm Phase 1 step 7)` | painter: `paintPreview.test.ts` "renders rotated-entities arm with stroke at rotated geometry"; tool-generator: `rotate.test.ts` "third prompt yields rotated-entities preview" |
| Move cursor in Rotate (post-base) | Ghost rotation angle updates per cursor frame | `packages/editor-2d/src/tools/runner.ts:67-89` existing subscription re-invokes `previewBuilder` per cursor tick (no Phase 2 change — reused) | tool-generator: `rotate.test.ts` "preview shape angleRad changes between two cursor positions" |
| Click final point in Rotate (no sub-option) | Selected primitives rotate about base by `atan2(click - base)`; tool exits | `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, point-input commit branch)` + `packages/domain/src/transforms/rotate.ts (NEW Phase 1 step 2)` | tool-generator: `rotate.test.ts` "commit rotates primitives by atan2 of click point"; domain-unit: `domain/tests/transforms.test.ts` "rotatePrimitive line by π/2 about origin → swaps x/y" |
| Type angle in DI pill (Rotate, third prompt) | Pill shows typed value; rubber-band freezes at typed angle on Tab | `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, dynamicInput field combineAs 'angle')` reuses B7 lock-gated effective cursor at `packages/editor-2d/src/tools/runner.ts:131-152` | tool-generator: `rotate.test.ts` "typed angle commits at locked rotation (DI feedInput angle)" |
| Press `R` sub-option at the third Rotate prompt | Reference sub-flow: yields `"Specify reference angle"` then `"Specify final angle"`; ghost stays at 0° until reference is picked | `packages/editor-2d/src/tools/rotate.ts (NEW Phase 2 step 1, subOption-input branch with 2 nested yields)` | tool-generator: `rotate.test.ts` "R sub-option opens 2-click reference-angle flow" |
| Press `MI` + Enter | Tool activates; "Select objects" (or first mirror-line prompt if pre-selected) | `packages/editor-2d/src/keyboard/shortcuts.ts:49-57` (MI entry NEW Phase 6) + `packages/editor-2d/src/tools/mirror.ts (NEW Phase 3 step 1)` | router: `keyboard-router.test.ts` "activates draw-mirror via MI shortcut" |
| Click first mirror line point | Prompt: "Specify second mirror line point"; live `'mirrored-entities'` ghost reflecting across line through (p1, cursor) | `packages/editor-2d/src/tools/mirror.ts (NEW Phase 3 step 1, second yield with previewBuilder)` + `packages/editor-2d/src/canvas/painters/paintPreview.ts:80-92 (NEW case 'mirrored-entities' Phase 1 step 8)` | painter: `paintPreview.test.ts` "renders mirrored-entities arm reflecting across line"; tool-generator: `mirror.test.ts` "second prompt yields mirrored-entities preview with live cursor" |
| Click second mirror line point | Prompt fires: `"Erase source objects? [Yes/No] <No>"` (sub-prompt) | `packages/editor-2d/src/tools/mirror.ts (NEW Phase 3 step 1, fourth yield with subOptions+defaultValue)` | tool-generator: `mirror.test.ts` "after second click yields erase-source sub-prompt with defaultValue 'No'" |
| Press Enter at erase-source prompt (no input — accepts default) | Mirrored entities added; sources kept | `packages/editor-2d/src/tools/mirror.ts (NEW Phase 3 step 1, post-fourth-yield No branch)` + `packages/project-store/src/actions/primitive-actions.ts:9-13` (`addPrimitive` reused) | tool-generator: `mirror.test.ts` "default Enter keeps source (no deletePrimitive call)" |
| Click [No] explicitly at erase-source prompt | Mirrored entities added; sources kept | same as above (No branch handles both default and explicit) | tool-generator: `mirror.test.ts` "explicit No subOption keeps source (no deletePrimitive call)" |
| Type `Y` or click [Yes] at erase-source prompt | Mirrored entities added; sources DELETED | `packages/editor-2d/src/tools/mirror.ts (NEW Phase 3 step 1, post-fourth-yield Yes branch)` + `packages/project-store/src/actions/primitive-actions.ts:25-32` (`deletePrimitive` reused) | tool-generator: `mirror.test.ts` "Yes subOption deletes source after addPrimitive" |
| Press `SC` + Enter, click base | Prompt: `"Specify scale factor or [Reference]"`; live `'scaled-entities'` ghost scaled by `factor = hypot(cursor - base)` | `packages/editor-2d/src/tools/scale.ts (NEW Phase 4 step 1, third yield with previewBuilder)` + `packages/editor-2d/src/canvas/painters/paintPreview.ts:80-92 (NEW case 'scaled-entities' Phase 1 step 8)` + `packages/domain/src/transforms/scale.ts (NEW Phase 1 step 3)` | painter: `paintPreview.test.ts` "renders scaled-entities arm with stroke at scaled geometry"; tool-generator: `scale.test.ts` "third prompt yields scaled-entities preview with cursor-distance factor" |
| Type number in DI pill (Scale, factor prompt) | Pill shows typed value; rubber-band scaled by typed factor on Tab | `packages/editor-2d/src/tools/scale.ts (NEW Phase 4 step 1, dynamicInput field combineAs 'number')` | tool-generator: `scale.test.ts` "typed factor commits at locked scale (DI feedInput number)" |
| Press `R` sub-option at Scale factor prompt | Reference sub-flow: yields `"Specify reference distance"` then `"Specify new distance"`; factor = `newDist / refDist` | `packages/editor-2d/src/tools/scale.ts (NEW Phase 4 step 1, subOption-input branch)` | tool-generator: `scale.test.ts` "R sub-option opens 2-click reference-distance flow" |
| Press `O` + Enter | Tool activates; "Select object to offset" (or skip if entity selected) | `packages/editor-2d/src/keyboard/shortcuts.ts:36-46` (O entry NEW Phase 6) + `packages/editor-2d/src/tools/offset.ts (NEW Phase 5 step 1)` | router: `keyboard-router.test.ts` "activates offset via O shortcut" |
| Click entity in Offset | Prompt: "Specify offset distance" with DI manifest (combineAs 'number') | `packages/editor-2d/src/tools/offset.ts (NEW Phase 5 step 1, second yield)` | tool-generator: `offset.test.ts` "second prompt yields offset-distance with DI number manifest" |
| Type distance, then prompt: "Specify point on side to offset" with live `'offset-preview'` ghost following cursor side | Side determined by sign of perpendicular projection of (cursor - source) onto source-normal | `packages/editor-2d/src/tools/offset.ts (NEW Phase 5 step 1, third yield with previewBuilder)` + `packages/editor-2d/src/canvas/painters/paintPreview.ts:80-92 (NEW case 'offset-preview' Phase 1 step 8)` + `packages/domain/src/transforms/offset.ts (NEW Phase 1 step 5)` | painter: `paintPreview.test.ts` "renders offset-preview arm at offset side"; tool-generator: `offset.test.ts` "third prompt yields offset-preview with live cursor side" |
| Click side in Offset | New offset entity created via `addPrimitive`; tool exits | `packages/editor-2d/src/tools/offset.ts (NEW Phase 5 step 1, commit branch)` + `packages/project-store/src/actions/primitive-actions.ts:9-13` (`addPrimitive`) | tool-generator: `offset.test.ts` "commit creates offset entity at distance × side via addPrimitive" |
| All 4 tools: Esc at any prompt | Tool aborts; preview clears; selection preserved | `packages/editor-2d/src/keyboard/router.ts:205-232` (existing two-step Esc, no per-tool change) — reused | tool-generator: `rotate.test.ts` "Esc at base-point aborts" + `mirror.test.ts` "Esc at first-mirror-point aborts" + `scale.test.ts` "Esc at base-point aborts" + `offset.test.ts` "Esc at distance-prompt aborts" (4 explicit tests, one per tool) |
...
| `packages/domain/src/transforms/scale.ts` | 1 | NEW. `scalePrimitive(p: Primitive, base: Point2D, factor: number): Primitive` — switch; uniform scale; arc/circle radius × factor; rectangle width/height × factor; xline pivot scales but angle unchanged (direction-invariant). **Per I-MOD-7 SSOT (§10):** `factor === 0` throws (degenerate); `factor < 0` allowed (AC flip — produces flipped-and-scaled result; e.g., line p1↔p2 about base when factor=-1). |
...
| `packages/editor-2d/src/tools/rotate.ts` | 2 | NEW. **Rev-2 single-prompt live-preview pattern.** Generator: `select → base → "Specify rotation angle or [Reference]"` (3 prompts default; +2 if Reference sub-option). Third prompt has `acceptedInputKinds: ['point', 'angle', 'subOption']`, `subOptions: [{ label: 'Reference', shortcut: 'r' }]`, `dynamicInput: { fields: [{ kind: 'angle', label: 'Angle' }], combineAs: 'angle' }`, `directDistanceFrom: base`, `previewBuilder` returns `{ kind: 'rotated-entities', primitives: ghost, base, angleRad: atan2(cursor - base) }` (live ghost from 0°). Branch on input kind: `'point'` → `deltaAngle = atan2(point - base)`; `'angle'` → `deltaAngle = radians`; `'subOption'`/Reference → 2-click reference-angle sub-flow then `deltaAngle = finalAngleRad - referenceAngleRad`. Commit: `for (id of selection) updatePrimitive(id, rotatePrimitive(p, base, deltaAngle))`. Emits one `'UPDATE'` operation per primitive (per A15 truth table). |
| `packages/editor-2d/src/tools/mirror.ts` | 3 | NEW. **Rev-2 with erase-source sub-prompt (AC parity).** Generator: `select → "Specify first mirror line point" → "Specify second mirror line point" → "Erase source objects? [Yes/No] <No>"` (4 prompts). Second prompt's `previewBuilder` returns `{ kind: 'mirrored-entities', primitives: ghost, line: { p1, p2: cursor } }`. Fourth prompt has `acceptedInputKinds: ['subOption']`, `subOptions: [{ label: 'Yes', shortcut: 'y' }, { label: 'No', shortcut: 'n' }]`, `defaultValue: 'No'`. Commit: `for (src of selection) addPrimitive(mirroredCopy(src, line))` (creates new entities); if `eraseInput.optionLabel === 'Yes'` then also `deletePrimitive(srcId)` per source. Default Enter without input → defaultValue 'No' → keep sources. Emits `'CREATE'` per mirror copy + optional `'DELETE'` per source (per A15 truth table). |
| `packages/editor-2d/src/tools/scale.ts` | 4 | NEW. **Rev-2 single-prompt live-preview pattern.** Generator: `select → base → "Specify scale factor or [Reference]"` (3 prompts default; +2 if Reference sub-option). Third prompt has `acceptedInputKinds: ['point', 'number', 'subOption']`, `subOptions: [{ label: 'Reference', shortcut: 'r' }]`, `dynamicInput: { fields: [{ kind: 'number', label: 'Factor' }], combineAs: 'number' }`, `directDistanceFrom: base`, `previewBuilder` returns `{ kind: 'scaled-entities', primitives: ghost, base, factor: hypot(cursor - base) }` (live ghost; AC convention factor = cursor distance). Branch: `'point'` → `factor = hypot(point - base)`; `'number'` → `factor = value`; `'subOption'`/Reference → 2-click sub-flow then `factor = newDist / refDist`. Reject `factor === 0` (per I-MOD-7 truth row). Negative factor allowed (AC convention: flips). Commit: `for (id of selection) updatePrimitive(id, scalePrimitive(p, base, factor))`. Emits `'UPDATE'` per primitive. |
| `packages/editor-2d/src/tools/offset.ts` | 5 | NEW. Generator: `select single entity → "Specify offset distance" → "Specify point on side to offset"`. Second prompt has `dynamicInput: { fields: [{ kind: 'distance', label: 'Distance' }], combineAs: 'number' }`. Third prompt's `previewBuilder` returns `{ kind: 'offset-preview', primitive: source, distance, side }` where `side = signOf(perpProjection(cursor - source))`. Commit: `addPrimitive(offsetPrimitive(source, distance, side))` — emits `'CREATE'` (per A15 truth table). Single-entity per V1 (multi-select deferred per A8). |
...
- **Undo/Redo:** each modify op emits operations per the §8.1 truth table below. zundo replays via existing operation log. Multi-primitive operations within one tool invocation produce N operations (one per affected primitive); undo currently steps through them one at a time (acceptable per ADR-023 single-action grouping; group-undo is M1.3c polish).
- **Sync (ADR-010):** unchanged — operations carry before/after snapshots per existing emitter.

### 8.1 Operation-emission truth table (Rev-3 SSOT per Codex Round-1 H1)

This table is the **single source of truth** for which OperationType each tool emits per affected primitive. A15 + §8 + §13 risk rows + per-tool implementation in §9 all reference this table; conflicting restatements elsewhere in the plan are a defect. Test gates per §11 verify.

| Tool | Per-primitive operation | Per-source side-effect | Rationale |
|---|---|---|---|
| Rotate | `'UPDATE'` (one per `id` in selection) | none | Each primitive's geometry mutates in place; identity preserved. `updatePrimitive(id, rotatePrimitive(p, base, deltaAngle))`. |
| Scale | `'UPDATE'` (one per `id` in selection) | none | Same — in-place geometry mutation. `updatePrimitive(id, scalePrimitive(p, base, factor))`. |
| Mirror | `'CREATE'` (one per mirrored copy added) | optional `'DELETE'` (one per source id, **only when `eraseInput.optionLabel === 'Yes'`** per A6 Rev-2) | Mirror produces a NEW reflected entity; the source is preserved by default. `addPrimitive(mirroredCopy(src, line))` per source; conditional `deletePrimitive(srcId)` per source on Yes. |
| Offset | `'CREATE'` (one per offset entity) | none | Single-entity in V1; one new parallel/offset primitive added. `addPrimitive(offsetPrimitive(source, distance, side))`. |

Undo behavior follows directly: a Rotate on N primitives undoes via N reverse-`'UPDATE'` operations. A Mirror on N primitives with `Yes` undoes via N reverse-`'DELETE'` (re-add sources) + N reverse-`'CREATE'` (delete mirrored copies) — 2N operations. Mirror with `No` undoes via just N reverse-`'CREATE'`.

Plan §13 "Scale negative-factor" risk row and §3 A5 / §4.1 scale row align with: `factor === 0` rejected (degenerate); `factor < 0` allowed (AC flip semantics). Codified as **I-MOD-7** in §10.
...
| MOD-P1-PreviewShapeArms | `rg -n "^\s*\| \{ kind: 'rotated-entities'\|^\s*\| \{ kind: 'scaled-entities'\|^\s*\| \{ kind: 'mirrored-entities'\|^\s*\| \{ kind: 'offset-preview'" packages/editor-2d/src/tools/types.ts` | exactly 4 matches (one per arm — anchored to discriminated-union member-line `\| { kind: 'X'` pattern; un-anchored `kind: 'X'` over-matches JSDoc + tests per Procedure 01 §1.8.1) |
| MOD-P1-PaintPreviewCases | `rg -n "^\s+case 'rotated-entities':\|^\s+case 'scaled-entities':\|^\s+case 'mirrored-entities':\|^\s+case 'offset-preview':" packages/editor-2d/src/canvas/painters/paintPreview.ts` | exactly 4 matches (anchored to switch-case-statement-line beginning with whitespace+`case 'X':`) |
...
| Gate | Command | Expected |
|---|---|---|
| MOD-P6-ToolIdUnion | `rg -n "^\s+\| 'rotate'\|^\s+\| 'mirror'\|^\s+\| 'scale'\|^\s+\| 'offset'" packages/editor-2d/src/keyboard/shortcuts.ts` | exactly 4 matches (anchored to ToolId union member lines `  \| 'X'`) |
| MOD-P6-ShortcutMaps | `rg -n "^\s+R: 'rotate',\|^\s+O: 'offset',\|^\s+MI: 'mirror',\|^\s+SC: 'scale',$" packages/editor-2d/src/keyboard/shortcuts.ts` | exactly 4 matches (anchored to object-literal property lines in SINGLE_LETTER_SHORTCUTS / MULTI_LETTER_SHORTCUTS) |
| MOD-P6-Registry | `rg -n "^\s+rotate: rotateTool,\|^\s+mirror: mirrorTool,\|^\s+scale: scaleTool,\|^\s+offset: offsetTool,$" packages/editor-2d/src/tools/index.ts` | exactly 4 matches (anchored to ESSENTIAL_REGISTRY object-property lines) |
...
| I-MOD-6 | Operation emission per §8.1 truth table (Rev-3 SSOT — Rotate/Scale `'UPDATE'`; Mirror `'CREATE'` + optional `'DELETE'`; Offset `'CREATE'`) | 2-5 | per-tool tests assert `emitOperation` mock calls match the truth-table row for the tool |
| I-MOD-7 | Scale factor domain: `factor === 0` rejected (degenerate, tool aborts); `factor < 0` allowed (AC flip semantics — produces a flipped-and-scaled result; combiner + tool both pass through). Single SSOT — A5 / §4.1 scale row / §13 risk row all agree. | 1, 4 | `domain/tests/transforms.test.ts` "scalePrimitive throws on factor=0"; "scalePrimitive flips on negative factor (line: p1↔p2 about base)"; tool-generator: `scale.test.ts` "factor=0 input aborts tool"; "factor=-2 commits flipped scale" |
...
| Scale factor domain — what's the semantics for `factor === 0` and `factor < 0`? | Codified as I-MOD-7 SSOT (§10): `factor === 0` rejected (tool aborts; `scalePrimitive` throws); `factor < 0` allowed (AC parity — negative scale flips and scales). Tests in `scale.test.ts` + `domain/tests/transforms.test.ts` cover both branches. No restatement in §4.1 or §13 — single source. |
